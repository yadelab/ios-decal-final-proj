...
class ViewController: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate {
   ...
   var categorizedOutput: [String: String] = [
    ...
    func recommendActivityType() {
        //if weather[0] == "snow" {
        //    self.recommendedActivity = "snow"
        //    return
        //}
        //Tried hardcoding but wouldn't search yelp correctly anyway
        self.recommendedActivity = "airsoft"
    }
    
    let app = UIApplication.shared
    
    //Could not get URL to open with specified location and category
    @IBAction func openYelp(_ sender: AnyObject) {
        self.recommendActivityType()
        let activity = self.recommendedActivity
        let city = cityTextField.text
        app.open(Applications.Yelp(), action: .open)
        //app.open(Applications.Yelp(), action: .searchCatLoc(loc: city, cat: activity))
    ...
    func analyzeWeather() {
        ...
        if (weather.contains("cloudy")) {
            self.weather.append("cloudy")
        }
        
        if (weather.contains("clear")) {
            self.weather.append("clear")
        }
        
        if (weather.contains("snow") || weather.contains("blizzard")) {
            self.weather.append("snow")
        }
        
        if (weather.contains("dry")) {
            self.weather.append("dry")
        }
    

        //Set temperature variable for recommendation
        //let regex = try! NSRegularExpression(pattern: "/([\s\S]\d\d)/")
        let regex = try! NSRegularExpression(pattern: "-?[0-9][0-9]|-?[0-9]"); //Used to find degrees (in Celsius)
            ...
            let task = URLSession.shared.dataTask(with: url, completionHandler: { (data, response, error) -> Void in
                ...
                if let urlContent = data {
                    ...
                    if (websiteArray?.count)! > 1 {
                        ...
                        self.weather = [] //Resets weather from previous searches
                        self.analyzeWeather()
                    }
